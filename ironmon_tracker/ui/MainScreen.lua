local function MainScreen(initialSettings, initialTracker, initialProgram)
    ...
    local function onPokemonHPHoverEnd()
        hoveringOverHP = false
        program.drawCurrentScreens()
    end

    local function onPokemonHPHover()
        local isEnemy = currentPokemon.owner == program.SELECTED_PLAYERS.ENEMY
        if randomBallPickerActive or isEnemy or inTrackedView or inPastRunView or currentPokemon.fromTeamInfoView then
            onPokemonHPHoverEnd()
            return
    ...
    local function setUpFriendshipBar(isEnemy)
        extraThingsToDraw.friendshipBarHover = nil
        if isEnemy or not hoveringOverHP or currentPokemon.fromTeamInfoView then
            return
        end
        local type1Position = ui.controls.pokemonHP.getPosition()
        local base = currentPokemon.baseFriendship or 0
        local progress = (currentPokemon.friendship - base) / (220 - base)
        extraThingsToDraw.friendshipBarHover = {
            ["progress"] = progress,
            x = type1Position.x + 2,
            y = type1Position.y + 4,
        }
    ...
    local function initEventListeners()
        ...
        table.insert(eventListeners, MouseClickEventListener(ui.controls.bookmarkIcon, onBookmarkClick))
        eventListeners.HPHoverListener =
            HoverEventListener(
