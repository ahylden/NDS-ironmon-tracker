...
function DrawingUtils.drawFriendshipBar(x, y, percent)
    local width = 61
    local multiplier = math.min(1, percent)
    local friendBarWidth = math.floor((width - 4) * multiplier)
    local outlineColor = DrawingUtils.convertColorKeyToColor("Top box border color")
    local friendBarColor = 0xFF00FFFF
    local darkenedFriendColor = 0xFF008585
    gui.drawRectangle(x + 1, y, width - 2, 3, outlineColor, nil)
    gui.drawRectangle(x, y + 1, 0, 1, outlineColor, outlineColor)
    gui.drawRectangle(x + width, y + 1, 0, 1, outlineColor, outlineColor)
    if friendBarWidth > 0 then
        gui.drawRectangle(x + 2, y + 2, friendBarWidth, 0, friendBarColor, friendBarColor)
        gui.drawRectangle(x + 2, y + 1, friendBarWidth, 0, darkenedFriendColor, darkenedFriendColor)
    end
end
...
function DrawingUtils.drawExtraMainScreenStuff(extraThingsToDraw)
    ...
    if extraThingsToDraw.friendshipBarHover ~= nil then
        local x, y, progress = 
            extraThingsToDraw.friendshipBarHover.x - 1,
            extraThingsToDraw.friendshipBarHover.y,
            extraThingsToDraw.friendshipBarHover.progress
        DrawingUtils.drawFriendshipBar(x, y, progress)
    end
